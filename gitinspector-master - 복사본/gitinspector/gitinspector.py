...
from . import (basedir, clone, extensions, filtering, format, help, interval,
               localization, optval, terminal, version, hello, zip)
...
def main():
	...
	try:
		opts, args = optval.gnu_getopt(argv[1:], "f:F:hHlLmrTwx:", ["exclude=", "file-types=", "format=",
		                                         ...
		                                         "timeline:true", "until=", "version", "weeks:true","hello"])

		oopt = int(input("1. Local Folder  2. Local Zip File  3. Git Address : "))

		if oopt == 1:
			directory = input("Path of Local Folder : ")
			os.chdir(directory)
		elif oopt == 2:
			zip_file = input("Local Zip File's Path and Name : ")
			zip.execute(zip_file)
			os.chdir(zip_file + 'Unzip')
		elif oopt == 3:
			git_adr = input("Git Repository Address : ")
			args.append(git_adr)
